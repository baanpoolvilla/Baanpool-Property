...
| Database  | PostgreSQL              |
| API       | PostgREST               |
| Client    | Supabase JS (for PostgREST connectivity) |

> **Note:** This project uses **PostgreSQL + PostgREST**, not Supabase cloud service. We use the `@supabase/supabase-js` client library because it provides a clean, type-safe API that works seamlessly with PostgREST endpoints.
...
Make sure to update the database connection in `docker-compose.postgrest.yml`:

```yaml
PGRST_DB_URI: "postgres://user:pass@host.docker.internal:5432/your_db"
```

**Option B: Direct Configuration**

Edit `postgrest.conf` with your database credentials:
...
Then run: `postgrest postgrest.conf`

### 3. Frontend

```bash
npm install
cp .env.example .env.local   # edit NEXT_PUBLIC_SUPABASE_URL if needed
npm run dev
```
...
|--------------------------------|-------------------------|--------------------------------------------------|
| `NEXT_PUBLIC_SUPABASE_URL`     | `http://localhost:3001` | PostgREST server URL                             |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY`| `placeholder`           | Any string (required by Supabase client library) |
